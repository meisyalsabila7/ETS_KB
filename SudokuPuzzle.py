...
class Board:
    ...
    def draw_board(self):
        for i in range(9):
            for j in range(9):
                if j % 3 == 0 and j != 0:  
                    pygame.draw.line(self.window, (255, 255, 255), (j // 3
                            * 180, 0), (j // 3 * 180, 540), 4)

                if i % 3 == 0 and i != 0: 
                    pygame.draw.line(self.window, (255, 255, 255), (0, i // 3
                            * 180), (540, i // 3 * 180), 4)

                self.tiles[i][j].draw((255, 255, 255), 1)

                if self.tiles[i][j].value != 0:  
                    self.tiles[i][j].display(self.tiles[i][j].value,
                            (21 + j * 60, 16 + i * 60), (255, 255, 255))  

        pygame.draw.line(self.window, (255, 255, 255), (0, (i + 1) // 3
                         * 180), (540, (i + 1) // 3 * 180), 4)
    ...
    def redraw(self, keys, wrong, time):
        ...
        if wrong > 0:
            ...
            text = font.render(str(wrong), True, (255, 255, 255))
...
def main():
    ...
    font = pygame.font.SysFont('Bahnschrift', 40)
    text = font.render('Generating Puzzles', True, (255, 255, 255))
...
text = font.render('Sudoku Solver', True, (255, 255, 255))
...
class button():
	...
	def draw_button(self):
		...
		pygame.draw.line(screen, (255, 255, 255), (self.x, self.y + self.height), (self.x + self.width, self.y + self.height), 2)
		pygame.draw.line(screen, (255, 255, 255), (self.x + self.width, self.y), (self.x + self.width, self.y + self.height), 2)
